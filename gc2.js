...
(function() {
  var Generator, Item, PlusMarker, animate_plus_markers, arceus, ball01, ball02, ball99, basedata, battle, bitfield_to_sstr, bitmap, cave, church, click1, cloninglab, cursor, daycare, export_save, farm, fountain, gcm1, gcm2, gcm3, gcm4, gcm5, gcminer, gen, generators, gens, goodra, goomy, hexstr, import_save, init_cooldown_time, init_input, item_ids, itemlist, items, last_update_time, load_save_from_local_storage, monkey, monkeyengineer, monkey01, monkey02, monkey03, ngens, photoncollider, plus_marker_anims, plus_marker_distance, plus_marker_id, plus_marker_origradius, plus_markers, recalc, recalc_gpc, recalc_gps, regenerate_tooltips, reserve, rngabuser, save_to_local_storage, shiny_goomy, sliggoo, sstr_to_bitfield, start_if_loaded, startup, supermonkey, trench, update, updateF, update_all_numbers, update_language, update_numbers, youngsterpocalypse, _i, _import_save_0_05, _import_save_0_10, _len,
  ...
  ngens = [daycare, reserve, farm, fountain, cave, trench, arceus, rngabuser, cloninglab, church, gcminer, photoncollider, monkeyengineer, supermonkey];
  ...
  battle = {
    ...
    bps_mult: 1,
    defeated: 0,
    names: ["Blue Bloon", "Green Bloon", "Yellow Bloon", "Pink Bloon", "Lead Bloon", "Ceramic Bloon", "MOAB Bloon"],
    start: function() {
      this.active = true;
      this.timer = 30000;
      this.max_hp = Math.max(200, basedata.gps * 40 + basedata.gpc * 80 + this.defeated * 250);
      this.hp = this.max_hp;
      return $("#battle_status").html("Boss fight started! Pop it before time runs out.");
    },
    tap: function() {
      if (!this.active) {
        return;
    ...
    win: function() {
      ...
      this.tap_mult += 0.05;
      this.bps_mult += 0.03;
      name = this.names[Math.min(this.names.length - 1, this.defeated - 1)];
      $("#battle_trophies").append("<div class='battle-trophy'>" + name + "<br/>+5% Tap, +3% BPS</div>");
      $("#battle_status").html("Victory! " + name + " defeated.");
      return recalc();
    },
    lose: function() {
      this.active = false;
      return $("#battle_status").html("Time up! The boss escaped.");
    },
    update: function(ms) {
      if (!this.active) {
        return;
    ...
    render: function() {
      ...
      $("#battle_hp").attr("max", this.max_hp || 100);
      return $("#battle_hp").attr("value", Math.max(0, this.hp));
    }
  };
  ...
  _import_save_0_10 = function(str) {
    var d02_basedata, d03_goomystats, d04_generators, d05_upgrades, d05_upgrades_bought, d05_upgrades_unlocked, data, datum, gen_data, generator_names, i, id, max_gens, save, save_time, _j, _k, _ref;
    save = {};
    data = str.split("||", 6);
    save_time = new Date();
    ...
    generator_names = ["cursor", "monkey", "daycare", "reserve", "farm", "fountain", "cave", "trench", "arceus", "rngabuser", "cloninglab", "church", "gcminer", "photoncollider", "monkeyengineer", "supermonkey"];
    gen_data = {};
  ...
  _import_save_0_05 = function(str) {
    var d02_basedata, d03_goomystats, d04_generators, data, datum, gen_data, generator_names, i, max_gens, save, save_time, _j, _ref;
    save = {};
    data = str.split("||", 5);
    save_time = new Date();
    save_time.setTime(parseInt(data[1]));
  ...
  recalc_gps = function() {
    ...
    if (items["monkey01"].bought) {
      gens["monkey"].gps *= 2;
      gens["daycare"].gps *= 4;
    }
    if (items["monkey02"].bought) {
      gens["monkey"].gps *= 2;
      gens["reserve"].gps *= 3;
    }
    if (items["monkey03"].bought) {
      gens["daycare"].gps *= 2;
      gens["reserve"].gps *= 2;
      gens["fountain"].gps *= 3;
    }
  ...
  */
  ...
  monkey01 = new Item("monkey01", 51);

  monkey01.unlock_condition = function() {
    return gens["daycare"].count >= 50 && gens["monkey"].count >= 100;
  };

  monkey01.cost = 6e6;
    
  monkey02 = new Item("monkey02", 52);

  monkey02.unlock_condition = function() {
    return gens["reserve"].count >= 50 && gens["daycare"].count >= 100 && gens["monkey"].count >= 200;
  };

  monkey02.cost = 5e10;
    
  monkey03 = new Item("monkey03", 53);

  monkey03.unlock_condition = function() {
    return gens["fountain"].count >= 50 && gens["reserve"].count >= 100 && gens["daycare"].count >= 200 && gens["monkey"].count >= 300;
  };

  monkey03.cost = 2.5e14;
